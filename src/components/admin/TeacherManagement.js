...
const TeacherTable = ({ teachers = [], onAssignClick, loading }) => {
  if (!Array.isArray(teachers)) {
    return (
      <div className="text-red-500">
        Invalid data format: teachers is not an array
  ...
  return (
    <div className="overflow-x-auto">
      <table className="min-w-full text-sm text-left text-gray-500 border border-gray-200">
        <thead className="bg-gray-100 text-xs uppercase text-gray-700">
          <tr>
            <th className="px-6 py-3 text-center text-[16px]">Name</th>
            <th className="px-6 py-3 text-center text-[16px]">Email</th>
            <th className="px-6 py-3 text-center text-[16px]">Role</th>
            <th className="px-6 py-3 text-center text-[16px]">Status</th>
            <th className="px-6 py-3 text-center text-[16px] text-nowrap">
              Class Teacher
            </th>
            <th className="px-6 py-3 text-center text-[16px]">
              Subject Assignments
            </th>
            <th className="px-6 py-3 text-center text-[16px]">Actions</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-200">
          {loading ? (
            ...
            teachers.map((teacher) => (
              <tr key={teacher._id} className="bg-white hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-center">{teacher.email}</td>
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-wrap justify-center gap-1">
                    {teacher.currentAssignments?.subjectTeacher?.map(
                      (subj, idx) => (
                        <span
                          key={idx}
                          className="bg-gray-200 text-gray-800 text-xs font-medium px-2 py-1 rounded"
                    ...
                    {(!teacher.currentAssignments?.subjectTeacher ||
                      teacher.currentAssignments.subjectTeacher.length ===
                        0) && (
