...
import { Add as AddIcon, Close as CloseIcon, Edit as EditIcon, Delete as DeleteIcon, ExpandMore as ExpandMoreIcon, ExpandLess as ExpandLessIcon } from '@mui/icons-material';
import FeesManagerLayout from '../../components/layout/FeesManagerLayout';
import feesService from '../../services/feesService';
import { useAuth } from '../../contexts/AuthContext';
...
const FeeDefinitions = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    classIds: [],
    dueDate: '',
    feeTypes: [{ type: '', amount: '', description: '', transportationSlab: '' }]
  });
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        ...
        const response = await feesService.getAvailableClasses();
        setClasses(response.data.classes || []);
        setFormData(prev => ({ ...prev, classIds: response.data.classes.map(c => c._id) }));
      } catch (err) {
        setError(err.response?.data?.message || 'Failed to load classes');
        console.error('Error fetching classes:', err);
      } finally {
  ...
  useEffect(() => {
    const fetchFeeDefinitions = async () => {
      try {
        setFetching(true);
        setError(null);
        const response = await feesService.getFeeDefinitions(year, classId || null);
        setFeeDefinitions(response.fees || {});
      } catch (err) {
        if (err.response?.status !== 404) {
          setError(err.response?.data?.message || 'Failed to load fee definitions');
  ...
  const addFeeType = () => {
    setFormData(prev => ({
      ...prev,
      feeTypes: [...prev.feeTypes, { type: '', amount: '', description: '', transportationSlab: '' }]
    }));
  };
  ...
  const validateForm = () => {
    ...
    for (let i = 0; i < formData.feeTypes.length; i++) {
      ...
      if (!fee.amount || isNaN(parseFloat(fee.amount)) || parseFloat(fee.amount) <= 0) {
        return `Valid amount is required for fee entry ${i + 1}`;
      }
      if (fee.type === 'transportation' && !fee.transportationSlab) {
        return `Distance slab is required for transportation fee entry ${i + 1}`;
      }
      const sameTypeSlab = formData.feeTypes.filter((f, j) => 
        j !== i && f.type === fee.type && 
        (f.type !== 'transportation' || f.transportationSlab === fee.transportationSlab)
      );
      if (sameTypeSlab.length > 0) {
        return `Duplicate fee type ${fee.type}${fee.transportationSlab ? ` (${fee.transportationSlab})` : ''} detected`;
      }
    }
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.error('Error saving fee definition:', err);
      let errorMessage = err.response?.data?.message || 'Failed to save fee definitions';
      if (err.response?.status === 400 && err.response?.data?.invalidClassIds) {
  ...
  const getDisplayAmount = (fee) => {
    ...
    if (fee.monthlyBreakdown) {
      const amounts = Object.values(fee.monthlyBreakdown).map(detail => detail.amount);
      const uniqueAmounts = [...new Set(amounts)];
      return uniqueAmounts.length === 1 ? `₹${uniqueAmounts[0].toLocaleString('en-IN')}` : 'Varies';
    }
    return 'N/A';
  };
  ...
  const getDisplayStatus = (fee) => {
    ...
    if (fee.monthlyBreakdown) {
      const statuses = Object.values(fee.monthlyBreakdown).map(detail => detail.status);
      const uniqueStatuses = [...new Set(statuses)];
      return uniqueStatuses.length === 1 ? uniqueStatuses[0].charAt(0).toUpperCase() + uniqueStatuses[0].slice(1) : 'Mixed';
    }
    return 'Unknown';
  };
  ...
  return (
    <FeesManagerLayout>
      <Box sx={{ mb: 4, px: { xs: 2, sm: 3, md: 4 }, bgcolor: 'background.default' }}>
        <Typography 
          variant="h4" 
          component="h1" 
          gutterBottom 
          ...
          <Grid item xs={12} sm={6} md={4}>
            <FormControl fullWidth size="small" sx={{ bgcolor: 'white', borderRadius: 1 }}>
              ...
              >
                ...
                {classes.map((cls) => (
                  <MenuItem key={cls._id} value={cls._id}>
                    {cls.name} {cls.division} {cls.academicYear ? `(${cls.academicYear})` : ''}
                  </MenuItem>
                ))}
              </Select>
            </FormControl>
      ...
      {fetching ? (
        ...
        <Card sx={{ mx: { xs: 2, sm: 3, md: 4 }, borderRadius: 2, boxShadow: 3 }}>
          <CardContent>
          ...
          <Table>
            <TableHead>
              <TableRow sx={{ bgcolor: 'grey.100' }}>
                ...
                <TableCell sx={{ fontWeight: 'bold', py: 1.5 }}>Fee Type</TableCell>
                <TableCell sx={{ fontWeight: 'bold', py: 1.5 }}>Class</TableCell>
                <TableCell sx={{ fontWeight: 'bold', py: 1.5 }}>Months</TableCell>
                <TableCell align="right" sx={{ fontWeight: 'bold', py: 1.5 }}>Monthly Amount</TableCell>
                <TableCell align="right" sx={{ fontWeight: 'bold', py: 1.5 }}>Annual Amount</TableCell>
                <TableCell sx={{ fontWeight: 'bold', py: 1.5 }}>Status</TableCell>
                <TableCell sx={{ fontWeight: 'bold', py: 1.5 }}>Description</TableCell>
                <TableCell sx={{ fontWeight: 'bold', py: 1.5 }}>Actions</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {Object.entries(feeDefinitions).map(([feeType, feeData], index) =>
                feeData?.map((fee, feeIndex) => {
                  const rowKey = `${feeType}-${fee.transportationSlab || ''}-${feeIndex}`;
                  ...
                  return (
                    <>
                      ...
                      >
                        <TableCell sx={{ py: 1 }}>
                          {fee.monthlyBreakdown && Object.keys(fee.monthlyBreakdown).length > 0 && (
                            <IconButton
                              size="small"
                        ...
                        <TableCell sx={{ py: 1 }}>
                          <Stack direction="row" alignItems="center" spacing={1}>
                            <Typography variant="body2">
                              {feeType && typeof feeType === 'string' 
                                ? feeType.charAt(0).toUpperCase() + feeType.slice(1)
                                : 'Unknown Type'}
                            </Typography>
                            {fee?.transportationSlab && (
                              <Chip 
                                label={fee.transportationSlab} 
                                size="small" 
                                color="primary" 
                                variant="outlined"
                                sx={{ borderRadius: 1 }}
                        ...
                        <TableCell sx={{ py: 1 }}>
                          {getDisplayMonths(fee)}
                        </TableCell>
                        <TableCell align="right" sx={{ py: 1 }}>
                          {getDisplayAmount(fee)}
                        </TableCell>
                        <TableCell align="right" sx={{ py: 1 }}>
                          ₹{fee?.annualAmount?.toLocaleString('en-IN') || 'N/A'}
                        </TableCell>
                        <TableCell sx={{ py: 1 }}>
                          <Chip 
                            label={getDisplayStatus(fee)}
                            color={getDisplayStatus(fee).toLowerCase() === 'pending' ? 'error' : 
                                  getDisplayStatus(fee).toLowerCase() === 'paid' ? 'success' : 
                                  getDisplayStatus(fee).toLowerCase() === 'partially_paid' ? 'warning' : 
                                  getDisplayStatus(fee).toLowerCase() === 'mixed' ? 'info' : 'default'}
                            size="small"
                            sx={{ borderRadius: 1 }}
                          />
                        </TableCell>
                                    ...
                                    {Object.entries(fee.monthlyBreakdown)
                                      ...
                                      .map(([month, details]) => (
                                        <TableRow key={`${rowKey}-${month}`}>
                                          ...
                                          <TableCell align="right">
                                            ₹{details.amount?.toLocaleString('en-IN') || 'N/A'}
                                          </TableCell>
                                          <TableCell>
                                            {details.status 
      ...
      >
              ...
              {formData.feeTypes.map((fee, index) => (
                ...
                >
                  <Grid container spacing={2}>
                    <Grid item xs={12} sm={6} md={4}>
                      <FormControl fullWidth size="small" sx={{ bgcolor: 'white', borderRadius: 1 }}>
                        <InputLabel id={`fee-type-label-${index}`}>Fee Type</InputLabel>
                        <Select
                    ...
                    {fee.type === 'transportation' && (
                      <Grid item xs={12} sm={6} md={4}>
                        <FormControl fullWidth size="small" sx={{ bgcolor: 'white', borderRadius: 1 }}>
                          <InputLabel id={`slab-label-${index}`}>Distance Slab</InputLabel>
                          <Select
                            labelId={`slab-label-${index}`}
                            value={fee.transportationSlab}
                            onChange={(e) => handleFormChange(index, 'transportationSlab', e.target.value)}
                            label="Distance Slab"
                            required
                            sx={{ '& .MuiSelect-select': { py: 1.5 } }}
                          >
                    ...
                    <Grid item xs={12} sm={6} md={fee.type === 'transportation' ? 4 : 8}>
                      <TextField
                        fullWidth
                        size="small"
                        label="Amount (per month)"
                      ...
                      <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                        <Tooltip title="Remove Fee Type">
                          <IconButton 
                            ...
                            disabled={formData.feeTypes.length === 1 || fetching}
                            sx={{ color: 'error.main' }}
                          >
                            <DeleteIcon />
                          </IconButton>
                ...
                >
                  {MONTHS.map((month, index) => (
                    <Chip
                      ...
                      onClick={() => handleMonthSelection(index + 1)}
                      color={selectedMonths.includes(index + 1) ? 'primary' : 'default'}
                      variant={selectedMonths.includes(index + 1) ? 'filled' : 'outlined'}
                      sx={{ 
                        m: 0.5, 
                        borderRadius: 1,
                        '&:hover': { 
                          bgcolor: selectedMonths.includes(index + 1) ? 'primary.dark' : 'grey.200'
                        }
                      }}
                    />
                  ))}
        ...
        <DialogActions sx={{ px: 3, py: 2, bgcolor: 'background.paper' }}>
          ...
          >
            {fetching ? <CircularProgress size={24} color="inherit" /> : editMode ? 'Update' : 'Create'}
          </Button>
        </DialogActions>
      </Dialog>
    </FeesManagerLayout>
